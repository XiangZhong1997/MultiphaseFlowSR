...
import multiphaseflowsr.benchmark.utils.symbolic_utils as su
import multiphaseflowsr.benchmark.utils.metrics_utils as metrics_utils
import multiphaseflowsr.benchmark.utils.timeout_unix  as timeout_unix
import multiphaseflowsr.benchmark.utils.read_logs     as read_logs
# Local imports
...
# ---------------------------------------------------- SCRIPT ARGS -----------------------------------------------------
parser = argparse.ArgumentParser (description     = "Analyzes Umf run results folder (works on ongoing benchmarks) "
...
# Grouped by correlation and keeping only essential columns
...
def timed_compare_expr(Umf_pb, trial_expr, verbose):
    return Umf_pb.compare_expression(trial_expr=trial_expr, verbose=verbose)
def untimed_compare_expr(Umf_pb, trial_expr, verbose):
    return Umf_pb.compare_expression(trial_expr=trial_expr, verbose=verbose)
def compare_expr(Umf_pb, trial_expr, verbose):
    if platform.system() == "Windows":
        return untimed_compare_expr(Umf_pb, trial_expr, verbose)
    else:
        return timed_compare_expr(Umf_pb, trial_expr, verbose)

def assess_equivalence (pareto_df, Umf_pb, check_only_most_acc = False, verbose = False):
    """
    ...
    pareto_df : pd.DataFrame
        Pareto front dataframe generated by multiphaseflowsr.
    Umf_pb : multiphaseflowsr.benchmark.UmfDataset.UmfCorrelation.UmfCorrelation
        Related Umf correlation.
    ...
    pareto_expressions = read_logs.get_pareto_expressions_from_df (pareto_df            = pareto_df,
                                                                   sympy_X_symbols_dict = Umf_pb.sympy_X_symbols_dict, )
    ...
    for i in rrange:
        ...
        if r > R_LIM:
            ...
            # Equivalence check
            try:
                is_equivalent, report = compare_expr(Umf_pb  = Umf_pb,
...
def assess_metric_test (pareto_df, Umf_pb, metric_func, i_pareto=-1):
    """
    ...
    pareto_expressions = read_logs.get_pareto_expressions_from_df(pareto_df            = pareto_df,
                                                                  sympy_X_symbols_dict = Umf_pb.sympy_X_symbols_dict, )
    # Generate test data
    X, y = pb.generate_data_points(uconfig.N_SAMPLES_TEST)
    y_target = y
...
def get_symbolic_result (pareto_df, Umf_pb, i_pareto = -1):
    """
...
t000 = time.time()
# ------- Results df -------
# Columns are from SRBench except for columns in upper cases which are added in the context of multiphaseflowsr:
# 'CORR NB'         : corr nb in Umf set (0 to 44 for type1 corrs, 45 to 88 for type2 corrs, 89 to 95 for type3 corrs and 96 to 114 for type4 corrs)
...
# Column name, dtype, [aggregation method across a single Umf correlation OR False if it should not be aggregated
...
umf_corr_indices = [0, 1, 36, 45, 47, 48, 56, 59, 69, 88, 89, 101]
# Iterating through Umf correlations
for i_corr in umf_corr_indices:
    ...
    for i_trial in range (N_TRIALS):
        ...
        # ----- Loading run data -----
        pareto_df, curves_df, noise_lvl = load_run_data(pb_folder_prefix = "UMF_%s_%s"%(i_corr, i_trial))
       ...
       # load_run_data returns nan noise_lvl if folder could not be found if it is not a NaN it should be consistent with script arg
        ...
        try:
            ...
            is_finished = n_evals >= (uconfig.MAX_N_EVALUATIONS - BATCH_SIZE- 1)
        except:
        ...
        if pareto_df is not None:
            ...
            _, equivalence_report, i_equivalent = assess_equivalence (pareto_df=pareto_df, Umf_pb=pb, verbose=True)
        else:
            i_equivalent = -1
            equivalence_report = {
                'symbolic_error'                : '',
        ...
        symbolic_result = get_symbolic_result(pareto_df=pareto_df, Umf_pb=pb, i_pareto=i_equivalent)
        ...
        if SAVE_UNFINISHED and (not is_finished):
            command = "python umf_run.py -i %i -t %i -n %f"%(i_corr, i_trial, noise_lvl)
        ...
        # evaluated, warn (as units may be inconsistent in Umf benchmark formulation)
        ...
        # mse_test
        try:
            mse_test = assess_metric_test (pareto_df = pareto_df, Umf_pb = pb, metric_func = metrics_utils.MSE, i_pareto=i_equivalent)
        ...
        # mae_test
        try:
            mae_test = assess_metric_test(pareto_df=pareto_df, Umf_pb=pb, metric_func=metrics_utils.MAE, i_pareto=i_equivalent)
            # If a Nan is returned (eg because of unprotected sqrt) go straight to except
        ...
        # r2_test
        try:
            r2_test = assess_metric_test(pareto_df=pareto_df, Umf_pb=pb, metric_func=metrics_utils.r2, i_pareto=i_equivalent)
            # If a Nan is returned (eg because of unprotected sqrt) go straight to except
        ...
        # r2_zero_test
        try:
            r2_zero_test = assess_metric_test (pareto_df = pareto_df, Umf_pb = pb, metric_func = metrics_utils.r2_zero, i_pareto=i_equivalent)
    ...
    results_agg_df = results_agg_df.set_index(results_agg_df["CORR NB"]).sort_index()
    results_agg_df.to_csv(PATH_RESULTS_SUMMARY_SAVE)
    # Saving at each iteration : aggregated summary -> essential
    results_essential_df = results_agg_df[["CORR NB", '# EVALUATIONS', 'STARTED', 'FINISHED', 'symbolic_solution', 'r2_test', 'r2_zero_test']]
...
total_evals_todo = n_runs * uconfig.MAX_N_EVALUATIONS

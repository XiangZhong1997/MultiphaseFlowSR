...
def ParallelExeAvailability(verbose=False):
    ...
    if mp_start_method == "spawn" and is_notebook:
        parallel_mode = False
        msg = "Parallel mode is not available because multiphaseflowsr is being ran from a notebook on a system returning " \
              "multiprocessing.get_start_method() = 'spawn' (typically MACs/Windows). Run multiphaseflowsr from the terminal to " \
              "use parallel mode."
        print(msg)
        warnings.warn(msg)

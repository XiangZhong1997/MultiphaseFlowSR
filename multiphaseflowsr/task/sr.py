...
# Internal imports
from multiphaseflowsr.config.config0 import config0
import multiphaseflowsr.learn.monitoring as monitoring
from multiphaseflowsr.task.fit import fit
import multiphaseflowsr.physym.execute as exec
...
def SR(X, y,
    ...
    (Wrapper around multiphaseflowsr.task.fit)
    ...
    op_names : array_like of shape (?) of str or None (optional)
        Names of choosable symbolic operations (see multiphaseflowsr.physym.functions for a list of available operations).
        By default, uses operations listed in multiphaseflowsr.task.sr.default_op_names.
    use_protected_ops : bool (optional)
        If True, uses protected operations (e.g. division by zero is avoided). True by default.
         (see multiphaseflowsr.physym.functions for a list of available protected operations).
    ...
    run_config : dict (optional)
        Run configuration (by default uses multiphaseflowsr.task.sr.default_config)
    get_run_logger : callable returning multiphaseflowsr.learn.monitoring.RunLogger (optional)
        Run logger (by default uses multiphaseflowsr.task.sr.get_default_run_logger)
    get_run_visualiser : callable returning multiphaseflowsr.learn.monitoring.RunVisualiser (optional)
        Run visualiser (by default uses multiphaseflowsr.task.sr.get_default_run_visualiser)
    ...
    Returns
    -------
    best_expression, run_logger : multiphaseflowsr.physym.program.Program, multiphaseflowsr.learn.monitoring.RunLogger
    ...
    run_config["learning_config"]["rewards_computer"] = multiphaseflowsr.physym.reward.make_RewardsComputer(**reward_config)
